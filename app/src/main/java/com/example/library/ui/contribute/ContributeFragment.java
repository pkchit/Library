...
public class ContributeFragment extends Fragment {
    ...
    private void saveBookInfoToDatabase()
    {
        ...
        BookRef.child(productRandomKey).setValue(new Contribution(productRandomKey,BookName,BookCategory,authorName,BookDescription,user.getEmail().replace(".","=*="),downloadImageUrl,null)).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
